...
export const AddDiaryEntryForm = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onSuccess();
      toast.success(
        isEditing ? "Запис успішно оновлено!" : "Запис успішно додано!"
      );
    },
    onError: () => {
      toast.error(
        isEditing ? "Не вдалося оновити запис." : "Не вдалося додати запис."
      );
    },
  });
  ...
  const handleSubmit = (
    ...
    actions: FormikHelpers<DiaryEntryFormValues>
  ) => {
  ...
  const handleDropdownClick = (e: React.MouseEvent) => {
    if (
      ...
      dropdownInputRef.current.contains(e.target as Node)
    ) {
  ...
  const toggleEmotion = (
    emotionId: string,
    setFieldValue: FormikHelpers<DiaryEntryFormValues>["setFieldValue"],
    values: DiaryEntryFormValues
  ) => {
    ...
    <>
      ...
      >
        {({ errors, touched, isSubmitting, values, setFieldValue }) => (
          <>
            <h3 className={css.from_modal_title}>Новий запис</h3>
            <Form className={css.form}>
              <div className={css.fieldWrap}>
                <label className={css.title} htmlFor="title">
                  Заголовок
                </label>
                <Field
              ...
              <div className={css.fieldWrap} ref={dropdownRef}>
                ...
                {!isDropdownOpen && values.emotions.length > 0 && (
                  <div
                    className={`${css.input} ${css.selectedTagsContainer} ${errors.emotions && touched.emotions ? css.inputError : ""}`}
                    onClick={handleDropdownClick}
                    ref={dropdownInputRef}
                  >
                    {values.emotions.map((emotionId) => {
                      const emotion = emotionsData?.find(
                        (e) => e._id === emotionId
                      );
                      return (
                    ...
                    <svg
                      width={24}
                      height={24}
                      className={`${css.dropdownArrow} ${isDropdownOpen ? css.dropdownArrowOpen : ""}`}
                    >
                ...
                {(isDropdownOpen || values.emotions.length === 0) && (
                  ...
                  >
                    <span
                      className={`${css.placeholder} ${errors.emotions && touched.emotions ? css.placeholderError : ""}`}
                    >
                      Оберіть категорію
                    </span>
                    <svg
                  ...
                  <div className={css.emotionsDropdownList}>
                    {emotionsData?.map((emotion) => (
                      <div
                        ...
                        onClick={() =>
                          toggleEmotion(emotion._id, setFieldValue, values)
                        }
                      >
                        <div
                          className={`${css.checkbox} ${values.emotions.includes(emotion._id) ? css.checked : ""}`}
                        >
              ...
              <div className={css.fieldWrap}>
                <label className={css.title} htmlFor="description">
                  Запис
                </label>
                <Field
                  className={`${css.textarea} ${errors.description && touched.description ? css.inputError : ""}`}
                  as="textarea"
                  name="description"
                  id="description"
                  rows={5}
